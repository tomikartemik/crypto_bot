...
func (t *Trader) trade() {
	...
	for _, instId := range configs.BotCurrentConfig.TradingPairs {
		...
		// Первый запуск - ждем смены тренда
...
func (t *Trader) monitorTrailingStop() {
	for _, instId := range configs.BotCurrentConfig.TradingPairs {
		...
		if !t.trailingActivated[instId] && dir*(price-t.positions[instId].EntryPrice) > atr*1.2 {
			...
			log.Printf("[Trader %s][%s] Активация трейлинг-стопа (%s) на %.02f", t.cfg.APIKey, instId, t.positions[instId].PosSide, t.trailingStopPrices[instId])
		}
		...
		if t.trailingActivated[instId] {
			if dir*(price-t.extremePrices[instId]) > 0 {
				...
				log.Printf("[Trader %s][%s] Подвинут трейлинг (%s) на %.02f", t.cfg.APIKey, instId, t.positions[instId].PosSide, t.trailingStopPrices[instId])
			}

			// Закрытие по трейлинг-стопу
			if dir*(price-t.trailingStopPrices[instId]) <= 0 {
				log.Printf("[Trader %s][%s] Закрыт %s по трейлинг-стопу на %.02f. Ждем смену тренда", t.cfg.APIKey, instId, t.positions[instId].PosSide, t.trailingStopPrices[instId])
		...
		if dir*(price-t.positions[instId].EntryPrice) <= -atr*configs.BotCurrentConfig.ATRMultiplierStopLoss {
			log.Printf("[Trader %s][%s] Сработал стоп-лосс %s на %.02f. Ждем смену тренда", t.cfg.APIKey, instId, t.positions[instId].PosSide, t.positions[instId].StopLossPrice)
			t.closePosition(instId)
		}
	}
}
